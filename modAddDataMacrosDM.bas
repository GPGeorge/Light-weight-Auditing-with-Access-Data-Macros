...
120       db.Execute "DELETE * FROM tblAuditLogConfig", dbFailOnError
...
140           If Left(tdef.Name, 3) = "tbl" _
                  ...
                  And tdef.Name <> "tblAuditLogConfig" _
              Then
...
280                       strSQL = "INSERT INTO tblAuditLogConfig (TableName, FieldName, FieldPosition, DataType, IsPrimaryKey, IsAuditable) " & _
                              "VALUES ('" & tdef.Name & "', '" & fld.Name & "', " & fld.OrdinalPosition & ", " & fld.type & ", " & isPK & ", " & -1 & ")" 'default to IsAuditable = true
290                       db.Execute strSQL, dbFailOnError
...
130       Set rs = db.OpenRecordset("SELECT TableName, FieldName, DataType, IsPrimaryKey FROM tblAuditLogConfig ORDER BY TableName, FieldName", dbOpenSnapshot)
...
'CurrentUser() will return "Admin" for all users, unless you use the now-obsolete object-level security that was available with Access 2003 and before.
'Alternatively you can use a function like GetAuditFieldsUserName() in the Northwind Dev Edition template.
